...
int calculRoulis(void){
		/* r?cup des valeurs sur les axes y et z */
		MySPI_Clear_NSS();
		...
		/** ci-apr?s le code permettant d'appeler la fonction retourRoulis pour controleVoile (cf. programme voiles) --> pour redresser le voilier
		if (roll > seuil){ //seuil valeur empirique que nous n'avons pas pu d?terminer
			redress=1;
		}
		else{
		**/
...
void initRoulis (void){
	...
	MySPI_Send(0xA);
	/* set de DATA_FORMAT ? 0 (right alignement of bytes)*/
